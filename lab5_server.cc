			...
			if (FD_ISSET(i, &read_fds)){
				if (i == sockfd) {	// sockfd로부터 메세지 온 경우
					...
					if ((numbytes = recv(i, buf, sizeof buf, 0)) <= 0){
						...
						strncpy(user_name, buf, user_idx);
						strcpy(user_list[i], user_name);
						strcpy(msg_buf, buf + (user_idx + 2));

						if (!strcmp(msg_buf, "/user")){
							char user_str[MAXDATASIZE];
							int count = 0;
							...
							for (int k = 4; k < count + 3; k++){ 
								strcpy(user_str + strlen(user_str), user_list[k]);
								if (k != count + 2)
									user_str[strlen(user_str)] = '\n';
							}

							if (send(i, user_str, strlen(user_str), 0) == -1)
								perror("send");
						}
						else {
							/*---------------lab4--------------*/
